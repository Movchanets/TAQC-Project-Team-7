...
> 24 |       await this.emailInput.waitFor({ state: 'visible', timeout: 5000 });
     |                             ^ TimeoutError: locator.waitFor: Timeout 5000ms exceeded.
  ...
  31 |   }
  32 | 
  33 |   async verifyErrorMessageVisible(): Promise<void> {
  34 |     await test.step('Verify that login error message is displayed', async () => {
  35 |       await expect(this.errorMessage).toBeVisible({ timeout: 7000 });
  36 |     });
  37 |   }
  38 | }
```