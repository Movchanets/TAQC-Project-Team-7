...
Call log:
  - waiting for locator('.header_sign-in-link:visible') to be visible
...
```ts
  ...
  11 |     
  12 |     this.loginButton = page.locator('.header_sign-in-link:visible');
  13 |   }
  14 | 
  15 |   async navigate() {
  16 |     await this.page.goto('#/greenCity');
  17 |   }
  18 | 
  19 |   async clickLogin() {
  20 |    
> 21 |     await this.loginButton.waitFor({ state: 'visible', timeout: 5000 });
     |                            ^ TimeoutError: locator.waitFor: Timeout 5000ms exceeded.
