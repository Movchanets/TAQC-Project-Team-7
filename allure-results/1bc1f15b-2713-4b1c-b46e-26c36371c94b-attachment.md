...
Call log:
  - waiting for locator('a.ubs-header-sign-in, .sign-in-link, button').filter({ hasText: /sign\s*in|увійти/i })
...
```ts
  ...
  12 |     this.page = page;
  13 | 
  14 |     this.signInButton = page.locator('a.ubs-header-sign-in, .sign-in-link, button').filter({ hasText: /sign\s*in|увійти/i });
  15 |     
  ...
  22 | 
  23 |       await this.signInButton.waitFor({ state: 'attached', timeout: 5000 });
  24 |       
> 25 |       await this.signInButton.click({ force: true });
     |                               ^ Error: locator.click: Element is not visible
  26 |     });
  27 |   }
  28 | 
  29 |   async clickEcoNewsButton(): Promise<void> {
  30 |     await allure.step('Click Eco News button in header', async () => {
  ...
  37 |       await this.homeButton.click();
  38 |     });
  39 |   }
  40 | }
```