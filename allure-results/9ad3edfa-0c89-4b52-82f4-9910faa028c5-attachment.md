...
Call log:
  - waiting for locator('.ubs-user-avatar, .user-avatar, [aria-label="sign in"], .header_user-name, img[alt="user icon"]').first() to be visible
...
```ts
  ...
  10 | 
  11 |     
  12 |     this.signInButton = page.locator('.ubs-user-avatar, .user-avatar, [aria-label="sign in"], .header_user-name, img[alt="user icon"]').first();
  13 |   }
  14 | 
  15 |   async clickSignInButton(): Promise<void> {
  16 |     await allure.step('Click Sign In button in header', async () => {
  17 |       
> 18 |       await this.signInButton.waitFor({ state: 'visible', timeout: 10000 });
     |                               ^ TimeoutError: locator.waitFor: Timeout 10000ms exceeded.
  19 |       
