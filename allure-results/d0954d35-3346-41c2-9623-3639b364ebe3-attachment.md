...
Call log:
  - waiting for locator('a, button, .ubs-header-sign-in, .sign-in-link').filter({ hasText: /sign\s*in|увійти/i }).filter({ visible: true }).first() to be visible
...
```ts
  ...
  12 |     this.page = page;
  13 | 
  14 |     this.signInButton = page.locator('a, button, .ubs-header-sign-in, .sign-in-link')
  15 |       .filter({ hasText: /sign\s*in|увійти/i })
  16 |       .filter({ visible: true })
  17 |       .first();
  18 |     
  19 |     this.ecoNewsButton = page.locator('a').filter({ hasText: /eco\s*news/i }).filter({ visible: true }).first();
  20 |     this.homeButton = page.locator('a').filter({ hasText: 'GreenCity' }).filter({ visible: true }).first();
  21 |   }
  22 | 
  23 |   async clickSignInButton(): Promise<void> {
  24 |     await allure.step('Click Sign In button in header', async () => {
  25 | 
> 26 |       await this.signInButton.waitFor({ state: 'visible', timeout: 5000 });
     |                               ^ TimeoutError: locator.waitFor: Timeout 5000ms exceeded.
  27 |   
  ...
  30 |   }
  31 | 
  32 |   async clickEcoNewsButton(): Promise<void> {
  33 |     await allure.step('Click Eco News button in header', async () => {
  34 |       await this.ecoNewsButton.click();
  35 |     });
  36 |   }
  37 | 
  38 |   async clickHomeButton(): Promise<void> {
  39 |     await allure.step('Click Home button in header', async () => {
  40 |       await this.homeButton.click();
  41 |     });
  42 |   }
  43 | }
```