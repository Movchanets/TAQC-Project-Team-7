...
Call log:
  - waiting for locator('.sign-in-link, button:has-text("Sign in"), [auth-component]')
...
```ts
  ...
  9  |     this.page = page;
  10 |     this.logo = page.locator('a.logo, .header_logo'); 
  11 |     this.loginButton = page.locator('.sign-in-link, button:has-text("Sign in"), [auth-component]');
  12 |   }
  13 | 
  14 |   async navigate() {
