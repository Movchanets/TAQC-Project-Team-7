...
Call log:
  - waiting for locator('li.sign-in-link, .sign-in-link, a:has-text("Sign in"), button:has-text("Sign in")').first() to be visible
    12 × locator resolved to hidden <a role="link" tabindex="0" _ngcontent-ng-c3629200733="" class="header_sign-in-link tertiary-global-button"> Sign in </a>
...
```ts
  ...
  11 |     
  12 |     // Оновлений селектор: шукає елемент із текстом "Sign in" або клас .sign-in-link всередині хедера
  13 |     this.loginButton = page.locator('li.sign-in-link, .sign-in-link, a:has-text("Sign in"), button:has-text("Sign in")').first();
  14 |   }
  15 | 
  16 |   async navigate() {
  ...
  19 | 
  20 |   async clickLogin() {
  21 |     // Чекаємо, поки кнопка з'явиться на екрані, і клікаємо
