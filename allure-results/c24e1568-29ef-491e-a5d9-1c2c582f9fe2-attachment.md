...
Call log:
  - waiting for locator('input[type="email"], input[id*="email"], input[name*="email"], input[placeholder*="email" i], input[placeholder*="імейл" i]') to be visible
...
```ts
  ...
  5  | 
  6  |   private get emailInput() { 
  7  |     return this.page.locator('input[type="email"], input[id*="email"], input[name*="email"], input[placeholder*="email" i], input[placeholder*="імейл" i]'); 
  8  |   }
  9  |   
  10 |   private get passwordInput() { 
  11 |     return this.page.locator('input[type="password"], input[id*="password"], input[name*="password"], input[placeholder*="password" i], input[placeholder*="пароль" i]'); 
  12 |   }
  13 |   
  14 |   private get submitButton() { 
  15 |     return this.page.locator('button[type="submit"], button:has-text("Sign in"), button:has-text("Увійти")'); 
  16 |   }
  17 |   
  18 |   private get errorMessage() { 
  19 |     return this.page.locator('.error-message, [class*="error"], .alert-danger, [role="alert"]').first(); 
  20 |   }
  21 | 
  22 |   async login(email: string, password: string): Promise<void> {
  23 |     await test.step(`Login with email: ${email}`, async () => {
  24 |      
> 25 |       await this.emailInput.waitFor({ state: 'visible', timeout: 10000 });
     |                             ^ TimeoutError: locator.waitFor: Timeout 10000ms exceeded.
  26 |     
  ...
  32 |   }
  33 | 
  34 |   async verifyErrorMessageVisible(): Promise<void> {
  35 |     await test.step('Verify that login error message is displayed', async () => {
  36 |       await expect(this.errorMessage).toBeVisible({ timeout: 10000 });
  37 |     });
  38 |   }
  39 | }
```