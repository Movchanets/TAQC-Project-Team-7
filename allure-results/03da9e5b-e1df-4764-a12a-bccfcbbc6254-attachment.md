...
Call log:
  - waiting for locator('input[type="email"], input[formcontrolname="email"]') to be visible
...
```ts
  ...
  7  |     return this.page.locator('input[type="email"], input[formcontrolname="email"]'); 
  8  |   }
  9  |   
  10 |   private get passwordInput() { 
  11 |     return this.page.locator('input[type="password"], input[formcontrolname="password"]'); 
  12 |   }
  13 |   
  14 |   private get submitButton() { 
  15 |     return this.page.locator('button[type="submit"]'); 
  16 |   }
  17 |   
  18 |   private get errorMessage() { 
  19 |     return this.page.locator('.error-message, [class*="error"], .alert-danger').first(); 
  20 |   }
  21 | 
  22 |   async login(email: string, password: string): Promise<void> {
  23 |     await test.step(`Login with email: ${email}`, async () => {
  24 | 
> 25 |       await this.emailInput.waitFor({ state: 'visible', timeout: 15000 });
     |                             ^ TimeoutError: locator.waitFor: Timeout 15000ms exceeded.
  ...
  36 |       
  37 |       await expect(this.errorMessage).toBeVisible({ timeout: 15000 });
  38 |     });
  39 |   }
  40 | }
```