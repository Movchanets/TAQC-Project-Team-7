...
Call log:
  - waiting for locator('.sign-in-link, a, button').filter({ hasText: /sign\s*in|увійти/i }).first() to be visible
    24 × locator resolved to hidden <a role="link" tabindex="0" _ngcontent-ng-c3629200733="" class="ubs-header-sign-in ng-star-inserted"> Sign in </a>
...
```ts
  ...
  6  | 
  7  |   readonly signInButton: Locator;
  8  |   readonly ecoNewsButton: Locator;
  9  |   readonly homeButton: Locator;
  10 | 
  11 |   constructor(page: Page) {
  12 |     this.page = page;
  13 | 
  14 |     this.signInButton = page.locator('.sign-in-link, a, button').filter({ hasText: /sign\s*in|увійти/i }).first();
  15 |     
  16 |     this.ecoNewsButton = page.locator('a').filter({ hasText: /eco\s*news/i }).first();
  17 |     this.homeButton = page.locator('a').filter({ hasText: 'GreenCity' }).first();
  18 |   }
  19 | 
  20 |   async clickSignInButton(): Promise<void> {
  21 |     await allure.step('Click Sign In button in header', async () => {
  22 |       // Чекаємо, поки кнопка точно з'явиться у DOM і стане видимою перед кліком
> 23 |       await this.signInButton.waitFor({ state: 'visible', timeout: 10000 });
     |                               ^ TimeoutError: locator.waitFor: Timeout 10000ms exceeded.
  24 |       await this.signInButton.click();
  25 |     });
  26 |   }
  27 | 
  28 |   async clickEcoNewsButton(): Promise<void> {
  29 |     await allure.step('Click Eco News button in header', async () => {
  30 |       await this.ecoNewsButton.waitFor({ state: 'visible', timeout: 5000 });
  31 |       await this.ecoNewsButton.click();
  32 |     });
  33 |   }
  34 | 
  35 |   async clickHomeButton(): Promise<void> {
  36 |     await allure.step('Click Home button in header', async () => {
  37 |       await this.homeButton.waitFor({ state: 'visible', timeout: 5000 });
  38 |       await this.homeButton.click();
  39 |     });
  40 |   }
  41 | }
```